...
class Pi:
    ...
    def run(self):
        ...
        self.rabbit.add_timer('pi_manifest', self.refresh_time, self.publish_manifest)
    ...
    def message_callback(self, message):
        ...
        if routing_key == START:
            self.on_start(phone, ip_addr, desc, region)
    ...
    def generate_manifest(self):
        ...
        # If after pruning AND implemeneting cooldown, we still have too many active records,
        # we trim the list. The records are ordered from most recent date_created, so we select
    ...
    def on_start(self, phone, ip_addr, region, desc):
        """
        ...
        with self.conn as conn:
            ...
            if record is None:
                ...
                dbapi.insert_record(conn, phone, ip_addr, region, desc)
                return
